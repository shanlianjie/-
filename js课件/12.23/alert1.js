...
Alert.prototype.createBtnBox = function(){
	...
	button1.onclick = this.action
	//给btn添加自定义属性，保存alert对象
	button1.that = this
...
Alert.prototype.action = function(){
	...
	//2. 执行回调函数
	//如果传过来的是个函数，才进行调用
	//this.that 从btn身上将alert对象取出来
	if(typeof this.that.callBack =="function"){
		//如果点击的是“取消”按钮，传0给回调函数，否则传1
		var index= this.innerText== "取消"?0:1
		this.that.callBack(index)
	}
}
